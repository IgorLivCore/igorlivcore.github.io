<?php
$EMAIL = 'support@plitprofi.ru'; //  Ваш e-mail на который будут приходить заявки
$FROM = '';  //  e-mail отправителя (иногда требуется указать разрешенный настройками e-mail)
$REPLY = 'support@plitprofi.ru';   //  e-mail для ответа 
...
function sendmail($_sSubject, $_sMessage, $_sEmail, $_sFrom, $_sReply, $_bPriority=false){
    ...
    $_sMessage = "Уведомление от plitprofi.ru: Заказан звонок пользователелем! <br>Имя:".stripinput($_POST['name'])."<br>"."Телефон:".stripinput($_POST['tel']);
